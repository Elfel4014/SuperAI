...
class CodeGenerator:
    def generate_simple_function(self, function_name, operation, a_name='a', b_name='b'):
        """
...
{body}"""
    ...
    def execute_and_test_code(self, code_string, test_cases):
        """
        Executa o código gerado e testa com os casos fornecidos.
        Retorna um dicionário com o status de aprovação e a saída dos testes.
        """
        local_vars = {}
        all_passed = True
        results = []
        ...
        try:
            ...
            for name, obj in local_vars.items():
                if inspect.isfunction(obj):
                    ...
                    break
            if func is None:
                return {"all_passed": False, "output": "Erro: Nenhuma função encontrada no código fornecido."}
            ...
            for inputs, expected_output in test_cases:
                try:
                    ...
                    if isinstance(expected_output, type) and issubclass(expected_output, Exception) and isinstance(e, expected_output):
                        ...
                        all_passed = False
            return {"all_passed": all_passed, "output": "\n".join(results)}
        except Exception as e:
            return {"all_passed": False, "output": f"Erro de execução do código: {e}"}

    def self_correct_code(self, original_code, error_message, function_name, operation, a_name='a', b_name='b'):
        """
