...
import urllib.parse
from superai_web_automation_module.web_navigator import fetch_url, parse_html_to_text, SimpleHTMLParser
...
def test_fetch_url_get(mock_urlopen):
...
def test_fetch_url_post(mock_urlopen):
    mock_response = MagicMock()
    ...
    data = {"key": "value"}
    content = fetch_url(url, data=data)
...
def test_parse_html_to_text():
    ...
    actual_text = parse_html_to_text(html_content)
